...
export function GoalBoard({ completed }: { completed: boolean }) {
	...
	const shown =
		filter === 'special-habit' ? store.specialHabits.filter(goal => goal.status === 'incomplete')
		: filter === 'all' ? goals
			...
			{!completed ?
				<Card>
					...
					<View style={uiStyles.wrap}>
						{[...DEFAULT_GOAL_CATEGORIES, ...customCategories].map(category => <Chip key={category} label={category} selected={newCategory === category} onPress={() => setNewCategory(category)} />)}
					</View>
					<View style={uiStyles.wrap}>
