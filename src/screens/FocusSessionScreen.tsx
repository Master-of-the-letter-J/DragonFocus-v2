...
const modeCopy: Record<FocusMode, { eyebrow: string; title: string; detail: string }> = {
	...
	wall: { eyebrow: 'QUIET FOCUS', title: 'Stare at a Wall', detail: 'The timer stays hidden. There is nothing to monitor.' },
};

export default function FocusSessionScreen() {
	const params = useLocalSearchParams<{ mode?: FocusMode; wallCountdown?: string }>();
	const mode: FocusMode = ['pomodoro', 'stopwatch', 'timer', 'wall'].includes(params.mode ?? '') ? params.mode! : 'pomodoro';
	const wallHasCountdown = mode === 'wall' && params.wallCountdown === 'true';
	...
	const hiddenMessage = mode !== 'wall' ? 'Timer hidden' : wallHasCountdown ? 'Hidden countdown active. Look away from the screen.' : 'Untimed quiet focus. Look away from the screen.';

	return (
		<SafeAreaView style={styles.safe}>
			<View style={styles.header}>
				<ActionButton compact tone="quiet" label="×" accessibilityLabel="Exit focus session" onPress={close} />
				<View style={styles.headerCopy}><Text style={styles.eyebrow}>{copy.eyebrow}</Text><Text style={styles.title}>{copy.title}</Text></View>
				<View style={styles.headerSpacer} />
			</View>
			<View style={styles.session}>
				<Animated.View entering={FadeIn.duration(450)} style={[styles.glow, glowStyle]} />
				<View style={styles.heart}><Text style={styles.heartGlyph}>♥</Text><Text style={styles.heartText}>{heart.toFixed(1)}%</Text></View>
				{timerHidden ? <Text style={styles.hiddenMessage}>{hiddenMessage}</Text> : <Text style={styles.timer}>{formatClock(seconds)}</Text>}
				<Text style={styles.detail}>{copy.detail}</Text>
				<View style={styles.heartBar}><ProgressBar value={heart} max={maximumHeart} color={colors.crimsonBright} label={`Crimson Heart · max ${maximumHeart.toFixed(0)}%`} /></View>
				{controlsHidden ? <ActionButton tone="quiet" label="Show controls" onPress={() => setControlsHidden(false)} /> : <Card style={styles.controls}>
					<View style={styles.actions}>
						<ActionButton label={pomodoro.isPaused ? 'Resume' : 'Pause'} onPress={pomodoro.isPaused ? pomodoro.resume : pomodoro.pause} />
						{mode !== 'wall' ? <ActionButton tone="quiet" label={timerHidden ? 'Show timer' : 'Hide timer'} onPress={() => setTimerHidden(value => !value)} /> : null}
					</View>
					{mode !== 'wall' && pomodoro.status !== 'count-up' ? <View style={styles.adjustments}>{[-600, -60, -10, 10, 60, 600].map(amount => <Chip key={amount} label={`${amount > 0 ? '+' : ''}${Math.abs(amount) >= 60 ? `${amount / 60}m` : `${amount}s`}`} onPress={() => pomodoro.adjustTime(amount)} />)}</View> : null}
					{mode === 'pomodoro' ? <View style={styles.actions}><ActionButton tone="secondary" label="Short break" onPress={() => { pomodoro.endSession(true); pomodoro.startBreak('short'); }} /><ActionButton tone="secondary" label="Long break" onPress={() => { pomodoro.endSession(true); pomodoro.startBreak('long'); }} /></View> : null}
					{mode === 'stopwatch' || mode === 'timer' ? <ActionButton compact tone="quiet" label="Hide controls" onPress={() => setControlsHidden(true)} /> : null}
				</Card>}
			</View>
		</SafeAreaView>
	);
...
const styles = StyleSheet.create({
	...
	hiddenMessage: { minHeight: 72, color: colors.muted, fontFamily: appFonts.medium, fontSize: 14, letterSpacing: 1, textAlign: 'center', textAlignVertical: 'center' },
