...
function FocusCave() {
	...
	const startSession = (mode: Exclude<typeof focusTab, 'heart'>) => {
		const started = mode === 'stopwatch' ? pomodoro.startCountUp() : pomodoro.startCountdown(25);
		if (started) router.push({ pathname: '/focus-session', params: { mode } });
	};
	return (
		<>
			...
			:	<Card accent={focusTab === 'pomodoro' || focusTab === 'timer' ? 'gold' : 'violet'}>
					<SectionTitle
						title={
							...
							: focusTab === 'timer' ?
								'25-minute timer'
							:	'25-minute quiet countdown'
						}
						detail={focusTab === 'wall' ? 'The countdown is always hidden during this session.' : 'Timer and control visibility can be changed inside the session.'}
					/>
					<ActionButton label={`Start ${focusTab === 'wall' ? 'quiet focus' : focusTab}`} onPress={() => startSession(focusTab)} />
				</Card>
			}
		</>
	);
